...
const PILL: Record<TipoDoc,{bg:string;tx:string;label:string}> = {
  FE:  { bg:'rgba(184,145,46,0.16)', tx:'#D8B24D', label:'Factura emitida'  },
  FR:  { bg:'rgba(186,117,23,0.18)', tx:'#BA7517', label:'Factura recibida' },
  BE:  { bg:'rgba(29,158,117,0.16)', tx:'#1D9E75', label:'Boleta'           },
  RET: { bg:'rgba(184,145,46,0.16)', tx:'#D8B24D', label:'Retención'        },
}
const ESTADO_STYLE: Record<Estado,{bg:string;tx:string}> = {
  pagada:   { bg:'rgba(29,158,117,0.16)', tx:'#1D9E75' },
  pendiente:{ bg:'rgba(186,117,23,0.18)', tx:'#BA7517' },
  vencida:  { bg:'rgba(226,75,74,0.16)', tx:'#E24B4A' },
}
    ...
    return (
      <>
        ...
        {lista.filter(d=>d.estado==='vencida').length>0 && (
          <div style={{ background:'rgba(226,75,74,0.16)', border:'1px solid rgba(226,75,74,0.22)', borderRadius:10, padding:'8px 12px', fontSize:12, color:'#E24B4A', marginBottom:12 }}>
        ...
        <div style={{ background:'#161616', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:'4px 20px', marginBottom:12 }}>
          ...
          {lista.map((d,i)=>{
            ...
            return (
              <div key={d.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 0', borderBottom:i<lista.length-1?'1px solid rgba(255,255,255,0.06)':'none', flexWrap:'wrap', gap:8 }}>
                <div style={{ flex:1, minWidth:180 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3, flexWrap:'wrap' }}>
                    <span style={{ fontSize:13, fontWeight:600, color:'#F0EFEA' }}>{d.folio}</span>
                    <span style={{ fontSize:10, padding:'1px 7px', borderRadius:999, fontWeight:600, background:pill.bg, color:pill.tx }}>{pill.label}</span>
                    <span style={{ fontSize:11, color:'#767676' }}>{empNombre(d.empresa_id)}</span>
                  </div>
                  <div style={{ fontSize:12, color:'#9A9A9A' }}>{d.contraparte} · RUT {d.rut_contraparte} · {d.fecha}</div>
                  <div style={{ fontSize:11, color:'#767676', marginTop:1 }}>Neto: {fmtCLP(d.neto)} · IVA: {fmtCLP(d.iva)}</div>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                  ...
                  <span style={{ fontSize:13, fontWeight:700, color:'#F0EFEA', whiteSpace:'nowrap' as const }}>{fmtCLP(d.total)}</span>
                  <button onClick={()=>eliminarDoc(d.id)} style={{ background:'transparent', border:'none', cursor:'pointer', fontSize:14, color:'#767676', padding:4 }}>🗑️</button>
                </div>
              </div>
            )
          })}
          {lista.length>0 && (
            <div style={{ display:'flex', justifyContent:'flex-end', gap:20, paddingTop:10, fontSize:12, color:'#9A9A9A' }}>
              <span>IVA: <strong style={{ color:'#F0EFEA' }}>{fmtCLP(lista.reduce((a,d)=>a+d.iva,0))}</strong></span>
              <span>Total: <strong style={{ color:'#F0EFEA' }}>{fmtCLP(lista.reduce((a,d)=>a+d.total,0))}</strong></span>
            </div>
          )}
        </div>
        {permitirNuevo && (
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: perfil } = await supabase
        ...
        .eq('email', email)
        .single()
      if (perfil?.modulos_permitidos && perfil.modulos_permitidos.length > 0 && !perfil.modulos_permitidos.includes('/tributario')) {
      ...
      <div className="app-content" style={{ marginLeft:220 }}>
        ...
        <div className="app-main" style={{ padding:'24px 28px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:12, marginBottom:24 }}>
            {[
              { label:'Docs emitidos',  value:emitidos.length,  sub:fmtM(emitidos.reduce((a,d)=>a+d.total,0)),  color:'#B8912E', bg:'rgba(184,145,46,0.16)' },
              { label:'Docs recibidos', value:recibidos.length, sub:fmtM(recibidos.reduce((a,d)=>a+d.total,0)), color:'#BA7517', bg:'rgba(186,117,23,0.18)' },
              { label:'IVA a pagar',    value:fmtM(ivaNeto),    sub:'débito − crédito', color:ivaNeto>0?'#E24B4A':'#1D9E75', bg:ivaNeto>0?'rgba(226,75,74,0.16)':'rgba(29,158,117,0.16)' },
              { label:'Vencidas',       value:vencidas,          sub:vencidas>0?'acción requerida':'al día', color:vencidas>0?'#E24B4A':'#1D9E75', bg:vencidas>0?'rgba(226,75,74,0.16)':'rgba(29,158,117,0.16)' },
            ].map(m=>(
          ...
          {/* Banner integración */}
          <div style={{ background:'rgba(184,145,46,0.16)', border:'1px solid rgba(184,145,46,0.4)', borderRadius:10, padding:'10px 16px', fontSize:12, color:'#B8912E', marginBottom:20, display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div style={{ display:'flex', gap:6, marginBottom:20, flexWrap:'wrap' }}>
            {([
              ...
              <button key={t.key} onClick={()=>{ setTab(t.key); setShowForm(false) }} style={{ padding:'6px 14px', borderRadius:8, fontSize:13, cursor:'pointer', border:'1px solid rgba(255,255,255,0.1)', background:tab===t.key?'rgba(184,145,46,0.16)':'#161616', color:tab===t.key?'#B8912E':'#9A9A9A', fontWeight:tab===t.key?500:400 }}>
                {t.label}
              </button>
            ))}
          </div>
              ...
              <div style={{ background:'#161616', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:20, marginBottom:14 }}>
                ...
                {[
                  ...
                  <div key={row.label} style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
                    <span style={{ fontSize:13, color:'#C9C9C9' }}>{row.label}</span>
                    <span style={{ fontSize:14, fontWeight:700, color:row.color }}>{row.value<0?'-':''}{fmtCLP(Math.abs(row.value))}</span>
                  </div>
                ))}
                <div style={{ display:'flex', justifyContent:'space-between', padding:'12px 0', borderTop:'2px solid rgba(255,255,255,0.1)', marginTop:4 }}>
                ...
                <div style={{ background:'rgba(186,117,23,0.18)', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#BA7517', marginTop:8 }}>
